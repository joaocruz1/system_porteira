...
export async function PUT(
  ...
  console.log("====== [APP ROUTER - PUT /api/custos/[id]] Atualizando produto no DB ======");
  ...
  let requestBody;
  try {
    requestBody = await request.json(); // Espera um corpo como { "valor": number }
    if (requestBody.valor === undefined || typeof requestBody.valor !== 'number') {
      return NextResponse.json<ErrorResponse>(
        { error: 'Corpo da requisição inválido: "valor" é obrigatória e deve ser um número.' },
        { status: 400 }
      );
    }
  } catch (e) {
    console.error('[API PUT /custos/[id]] Erro ao parsear corpo da requisição JSON:', e);
    return NextResponse.json<ErrorResponse>(
  ...
  try {
    ...
    const updatedCustos = await prisma.custos.update({
      where: {
        id: id, // 'id' deve ser o nome do campo identificador no seu modelo Custos
      },
      data: {
        valor: valor, // Campo a ser atualizado
        // Se você precisar atualizar outros campos, adicione-os aqui
        // Por exemplo: nome: requestBody.nome (se enviado no corpo)
      },
    });

    console.log("[DB UPDATE] Produto atualizado com sucesso no banco de dados:", updatedCustos);
    return NextResponse.json(updatedCustos, { status: 200 });

  } catch (error: unknown) {
    let errorMessage = 'Erro desconhecido ao atualizar custos no banco de dados.';
    let errorStatus = 500;

    console.error('[DB UPDATE] Erro ao atualizar custos:', error);
    ...
    if (error instanceof Error && 'code' in error) {
      ...
      if (prismaError.code === 'P2025') {
        errorMessage = `Custo com ID "${id}" não encontrado para atualização.`;
        errorStatus = 404; // Not Found
        console.warn(`[DB UPDATE] Tentativa de atualizar custos não existente. ID: ${id}`);
      } else {
...
// Se você tiver outras funções HTTP (GET, DELETE, POST para criar na coleção) para /api/custos/[id]
// elas podem coexistir neste mesmo arquivo.
// Por exemplo, se o DELETE que criamos antes fosse para /api/custos/[id] em vez de /api/produto/delete/[id],
// ele estaria aqui também.
// Para manter consistência com a rota do DELETE que usa /produto/delete/[id],
// esta rota PUT está em /api/custos/[id].
